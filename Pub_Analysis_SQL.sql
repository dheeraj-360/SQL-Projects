...
CREATE TABLE ratings ( rating_id INT PRIMARY KEY, pub_id INT, customer_name VARCHAR(50), rating FLOAT, 
...
select p.pub_name,(s.quantity*b.price_per_unit) as total_sales,s.transaction_date from
pubs p inner join sales s ON p.pub_id = s.pub_id
inner join beverages b ON b.beverage_id = s.beverage_id 
)
select * , sum(total_sales) OVER(PARTITION BY pub_name order by transaction_date RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) 
as sum_tot  from cte_total_sales


--FRAMING CLAUSE

RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW / UNBOUNDED FOLLOWING /N following
ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW / UNBOUNDED FOLLOWING /N following


--select pub_name, sum(total_sales) from cte_total_sales group by pub_name
