...
with cte as(
select  product_id, sum(quantity) as no_of_items_sold,DENSE_RANK() over(order by sum(quantity) desc ) as drnk , RANK() over(order by sum(quantity) desc ) as rnk ,
ROW_NUMBER() over(order by sum(quantity) desc ) as rn from transactions
group by product_id 
)

select s.product_name, c.no_of_items_sold from cte c inner join sustainable_clothing s 
...
with on_mc as(
  select avg(t.quantity*1.0) as avg_quan_during_mc
...
non_mc as(
  select avg(t.quantity*1.0) as avg_quan_not_in_mc
  from marketing_campaigns mc join transactions t on mc.product_id=t.product_id
  join sustainable_clothing sc on t.product_id=sc.product_id
  where t.purchase_date not between mc.start_date and mc.end_date
)

select greatest(avg_quan_during_mc,avg_quan_not_in_mc) as higheset_average from on_mc
cross join non_mc;
...
select  case when t.purchase_date between mc.start_date and mc.end_date then avg(quantity*1.0) over() end as avg_mc

from 
marketing_campaigns mc inner join transactions t 
ON mc.product_id = t.product_id;


with cte as(

select t.quantity , case when t.purchase_date between mc.start_date and mc.end_date then 'mc' else 'non-mc' end as camp
from 
marketing_campaigns mc inner join transactions t 
ON mc.product_id = t.product_id)


select camp,avg(quantity*1.0) as avg_quantity_sold from cte 
group by camp


select * from sustainable_clothing sc left join  marketing_campaigns mc  On sc.product_id = mc.product_id
where mc.product_id = 2
